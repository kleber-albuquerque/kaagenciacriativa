...
# 1. Remove QUALQUER script que contenha dograh, ngrok, ka-widget ou toggleRec
html = re.sub(r'(?is)<script[^>]*>.*?(dograh|ngrok|ka-widget|toggleRec|let recorder).*?</script>', '', html)

# 2. Remove divs e audios antigos do widget que possam ter sobrado
html = re.sub(r'(?is)<div[^>]*id=["\']ka-widget["\'].*?</div>', '', html)
html = re.sub(r'(?is)<audio[^>]*id=["\']ka-audio["\'][^>]*></audio>', '', html)

# 3. O Widget Limpo e Único
widget = """
<!-- Widget de Voz IA da Agência KA (Versão Final Limpa) -->
      ...
      try {
        ...
        recorder.onstop = async () => {
          ...
          try {
            ...
            status('Erro ao processar. Veja o console.');
        ...
        status('Ouvindo...');
      } catch (e) { status('Erro no mic'); console.error(e); }
    } else {
...
print("✅ Limpeza nuclear concluída! Apenas 1 widget limpo foi inserido.")
